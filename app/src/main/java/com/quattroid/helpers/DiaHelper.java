...
public class DiaHelper {
    ...
    public static void CalcularHorasDia(DatosDia datosDia, ArrayList<ServicioDia> servicios, BaseDatos datos) {
        ...
        HorasServicioModel[] servs = new HorasServicioModel[num];
        servs[0] = new HorasServicioModel();
        servs[0].Inicio = HoraHelper.horaToInt(datosDia.getInicio());
        servs[0].Final = HoraHelper.horaToInt(datosDia.getFinal());
        for (int m = 1; m < num; m++) {
            ServicioDia sss = servicios.get(m - 1);
            servs[m] = new HorasServicioModel();
            servs[m].Inicio = HoraHelper.horaToInt(sss.getInicio());
            servs[m].Final = HoraHelper.horaToInt(sss.getFinal());
        }
        EstadoDiaModel resultado = CalculosHelper.TiempoTrabajado(servs, datos.opciones, datosDia.getTurno(), datosDia.getTipoIncidencia());
        if (resultado == null) {
