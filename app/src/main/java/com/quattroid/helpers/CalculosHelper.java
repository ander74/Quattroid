...
import com.quattroid.models.EstadoDiaModel;
import com.quattroid.models.HorasServicioModel;

import org.joda.time.Days;
import org.joda.time.LocalDate;

import BaseDatos.Opciones;
...
public class CalculosHelper {
    ...
    /**
     * Esta versión utiliza un objeto SharedPreferences.
     */
    public static EstadoDiaModel TiempoTrabajado(HorasServicioModel[] horas,
                                                 Context context,
                                                 int turno,
                                                 int tipoIncidencia) {
        ...
        double limEntreServicios = HoraHelper.horaToDecimal(LimiteEntreServicios);
        double iniNocturnas = HoraHelper.horaToDecimal(InicioNocturnas);
        double finNocturnas = HoraHelper.horaToDecimal(FinalNocturnas);
        double desayun = HoraHelper.horaToDecimal(Desayuno);
        double com1 = HoraHelper.horaToDecimal(Comida1);
        double com2 = HoraHelper.horaToDecimal(Comida2);

        // Evaluamos la cena
        if (Cena < Desayuno) Cena += 1440;
        // NUEVO
        double cen = HoraHelper.horaToDecimal(Cena);
        ...
        double priInicio = HoraHelper.horaToDecimal(PrimerInicio);
        double ultFinal = HoraHelper.horaToDecimal(UltimoFinal);
        ...
        for (double M = 0.01; M <= minTotales; M = M + 0.01) {
            ...
            for (HorasServicioModel hora : horas) {
                ...
                if ((hora.Inicio != hora.Final) && (hora.Inicio != -1) && hora.Final != -1) {
                    ...
                    // NUEVO
                    double horaInicio = HoraHelper.horaToDecimal(hora.Inicio);
                    double horaFinal = HoraHelper.horaToDecimal(hora.Final);
        ...
        EstadoDiaModel resultado = new EstadoDiaModel();
    ...
    /**
     * Esta versión utiliza un objeto Opciones en lugar de un objeto SharedPreferences.
     */
    public static EstadoDiaModel TiempoTrabajado(HorasServicioModel[] horas,
                                                 Opciones opciones,
                                                 int turno,
        ...
        // Obtenemos las variables del convenio desde las preferencias.
        double Jornada = opciones.getJornadaMedia();
        double JornadaMinima = opciones.getJornadaMinima();
        int LimiteEntreServicios = opciones.getLimiteEntreServicios();
        int InicioNocturnas = opciones.getInicioNocturnas();
        int FinalNocturnas = opciones.getFinalNocturnas();
        int Desayuno = opciones.getLimiteDesayuno();
        int Comida1 = opciones.getLimiteComida1();
        int Comida2 = opciones.getLimiteComida2();
        int Cena = opciones.getLimiteCena();
