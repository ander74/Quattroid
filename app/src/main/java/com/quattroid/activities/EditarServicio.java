...
public class EditarServicio extends Activity implements AdapterView.OnItemClickListener,
    ...
    ArrayList<com.quattroid.data.models.ServicioAuxiliarModel> serviciosAuxiliares = null;
    Servicio serv = null;
    String lineatext = "";
    int id = -1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (id == -1) {
            ...
            if (serv.getEuros() == 0) {
                ...
                euros.setText(HoraHelper.textoDecimal(serv.getEuros()));
            }
            bloquearServicio();
        }
        ...
        serviciosAuxiliares = datos.getServiciosAuxiliaresNuevo(serv.getLinea(), serv.getServicio(), serv.getTurno());
...
//    }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        com.quattroid.data.models.ServicioAuxiliarModel servicioPulsado = serviciosAuxiliares.get(position);
    ...
    public void onFocusChange(View v, boolean hasFocus) {
        ...
        switch (v.getId()) {
            case R.id.et_servicio:
                servicio.setText(HoraHelper.validarServicio(servicio.getText().toString()));
                break;
            case R.id.et_turno:
                ...
                break;
            case R.id.et_inicio:
                inicio.setText(HoraHelper.horaToString(inicio.getText().toString()));
                break;
            case R.id.et_final:
                fin.setText(HoraHelper.horaToString(fin.getText().toString()));
                break;
            case R.id.et_tomaDeje:
                tomaDeje.setText(HoraHelper.horaToString(tomaDeje.getText().toString()));
                break;
            case R.id.et_euros:
                euros.setText(HoraHelper.validaHoraDecimal(euros.getText().toString()));
    ...
    public void onItemCheckedStateChanged(ActionMode mode, int position, long id, boolean checked) {
        ...
        if (checked) {
            ...
            serviciosAuxiliares.get(position).setSelected(true);
        } else {
            Integer pos = position;
            listaIds.remove(pos);
            serviciosAuxiliares.get(position).setSelected(false);
        }
    ...
    public void onDestroyActionMode(ActionMode actionMode) {
        ...
        desactivarBoton(botonBorrarServicioAuxiliar);
        //Refrescar la lista.
        for (com.quattroid.data.models.ServicioAuxiliarModel d : serviciosAuxiliares) {
            d.setSelected(false);
        }
        listaIds.clear();
        adaptador.notifyDataSetChanged();
    ...
    private void actualizarLista() {
        ...
        listaServicios.setAdapter(adaptador);
        adaptador.notifyDataSetChanged();
        CalculosHelper.setAlturaLista(listaServicios);
        if (serviciosAuxiliares.isEmpty()) {
            complementarios.setText("No hay servicios complementarios");
            if (id == -1) {
                desbloquearServicio();
    ...
    private void validarCampos() {
        ...
        s = HoraHelper.horaToString(tomaDeje.getText().toString().trim());
        if (s.equals("")) {
            serv.setTomaDeje("");
        } else {
            serv.setTomaDeje(s);
        ...
        s = HoraHelper.validaHoraDecimal(euros.getText().toString().trim());
        if (s.equals("")) {
            serv.setEuros(0d);
        } else {
    ...
    private void botonBorrarServicioAuxiliarPulsado(View view) {
        ...
        aviso.setPositiveButton("SI", (dialog, which) -> {
            ...
            ids.addAll(listaIds);
            for (int id : ids) {
                com.quattroid.data.models.ServicioAuxiliarModel servSeleccionado = serviciosAuxiliares.get(id);
