...
public class AdaptadorDiaCalendario extends ArrayAdapter<DatosDia> {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        holder.DiaSemana.setText(HoraHelper.DIAS_SEMANA_ABREV[Dsemana]);
        ...
        if (!s.equals("")) {
            if (tipo > 0 && tipo < 4 && dia.getTurno() != 0) {
                holder.Nocturnas.setText(HoraHelper.textoDecimal(hnoct));
                if (hacum > -0.01) {
                    holder.Acumuladas.setTextColor(Colores.VERDE_OSCURO);
                } else {
                    holder.Acumuladas.setTextColor(Colores.ROJO);
                }
                holder.Acumuladas.setText(HoraHelper.textoDecimal(hacum));
